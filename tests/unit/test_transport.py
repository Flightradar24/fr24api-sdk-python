...
from respx import MockRouter, mock as respx_mock # Import mock for decorator
...
def request_url_in_exc_message(exc: Fr24SdkError, expected_url: str) -> bool:
    ...
    if hasattr(exc, 'request') and exc.request is not None and hasattr(exc.request, 'url'):
...
def test_successful_request(method: str): # Removed respx_router fixture from args
    ...
    route = getattr(respx_mock, method.lower())(FULL_TEST_URL).respond(status_code=200, json=expected_response_json)

    # No `with respx_router:` needed as @respx.mock handles it
    client_for_test = httpx.Client(base_url=DEFAULT_BASE_URL) 
    transport_instance = HttpTransport(api_token=TEST_TOKEN, http_client=client_for_test)
...
def test_request_with_params_and_custom_headers(): # Removed respx_router fixture
    ...
    route = respx_mock.get(FULL_TEST_URL, params=params).respond(status_code=200, json={})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (402, {"message": "Payment Required"}, PaymentRequiredError, "Payment Required"),
        (403, {"message": "Forbidden"}, ApiError, "Forbidden"),
        (404, {"message": "Not Found"}, NotFoundError, "Not Found"),
        (429, {"message": "Rate limit exceeded"}, RateLimitError, "Rate limit exceeded"),
...
def test_api_error_mapping(
    ...
    route = respx_mock.get(FULL_TEST_URL).respond(status_code=status_code, json=error_json)
...
@pytest.mark.parametrize(
    ...
    [
        (httpx.TimeoutException("Timeout!", request=httpx.Request("GET", FULL_TEST_URL)), TransportError, "Request timed out"),
        (httpx.ConnectError("Cannot connect!", request=httpx.Request("GET", FULL_TEST_URL)), TransportError, "Request failed"),
        (httpx.NetworkError("Network issue!", request=httpx.Request("GET", FULL_TEST_URL)), TransportError, "Request failed"),
    ],
)
...
def test_transport_context_manager(): # respx_router fixture removed
    ...
    with HttpTransport(api_token=TEST_TOKEN, http_client=raw_client_passed_to_transport) as transport_instance:
        transport_instance.request("GET", TEST_API_ENDPOINT_PATH) 
...
def test_transport_explicit_close(): # respx_router fixture removed
    ...
    raw_client_passed_to_transport = httpx.Client(base_url=DEFAULT_BASE_URL)
    transport_instance = HttpTransport(api_token=TEST_TOKEN, http_client=raw_client_passed_to_transport)
