...
def main():
    ...
    with Client() as client:
        ...
        print(
            client.flight_summary.get_light(
                flights=["KL1316"],
                flight_datetime_from=datetime(2025, 5, 13, 12, 0, 0),
                flight_datetime_to=datetime(2025, 5, 14, 17, 10, 0),
            )
            .data[0]
            .fr24_id
        )
        ...
        print(
            client.live.flight_positions.get_light(
                bounds=Boundary(north=55.6, south=55.5, west=12.5, east=12.6)
            )
        )
        print(client.live.flight_positions.get_full(bounds="55.6,55.5,12.5,12.6"))
        print(
            client.historic.flight_positions.get_light(
                altitude_ranges=[
                    AltitudeRange(min_altitude=1000, max_altitude=10000),
                    "10000-20000",
                ],
                timestamp=datetime(2025, 5, 14, 18, 15, 0, tzinfo=timezone.utc),
            )
            .data[0]
            .fr24_id
        )
        print(
            client.historic.flight_positions.get_full(
                flights=["SK2752"],
                timestamp=datetime(2025, 5, 14, 18, 15, 0, tzinfo=timezone.utc),
            ).data[0]
        )
