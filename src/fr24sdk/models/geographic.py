...
class Boundary(BaseModel):
    ...
    def validate_boundary_logic(self):
        ...
        if self.north <= self.south:
            raise ValueError(f"North latitude ({self.north}) must be greater than south latitude ({self.south})")
        return self
    ...
    def from_string(cls, bounds_str: str) -> "Boundary":
        ...
        if len(parts) != 4:
            raise ValueError("Boundary string must have exactly 4 comma-separated values")
...
class AltitudeRange(BaseModel):
    ...
    def validate_altitude_range(self):
        ...
        if self.min_altitude > self.max_altitude:
            raise ValueError(f"Minimum altitude ({self.min_altitude}) must be <= maximum altitude ({self.max_altitude})")
        return self
    ...
    def from_string(cls, range_str: str) -> "AltitudeRange":
        ...
        if len(parts) != 2:
            raise ValueError("Altitude range string must have exactly 2 dash-separated values")
